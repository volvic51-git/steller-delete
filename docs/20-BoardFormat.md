...
| `startCells` | `{r,c}[]` | ✅ | 開始セル。**スキーマは配列**（1個以上）だが、**当面のポリシーは正確に1個**（§startCells）。 |
...
#### ポリシー：当面は正確に1個（スキーマは配列のまま）

- **スキーマは配列を維持する**（`[{r,c}]`）。スカラー化（`startCell`）はしない。
- **運用上は要素ちょうど1個に固定する。** 理由：大型盤面では入口の位置が体験をほぼ変えず
  （人間は大盤面を記憶・戦略化できない）、複数入口は検証コストを N 倍にするだけで益が薄い。
- **なぜ配列を残すか**（Evolution Principle）：将来イベント盤面等で複数入口が要るとき、
  フォーマットを変えずに対応できる。後から配列化するのは破壊的変更、最初から配列なら無痛。
- **検証との結合**：`verification.level=2`（推測なし完全クリア保証）は
  「**この単一 startCell から推測なしで解ける**」を意味する。入口が1個なので検証も1回で済む。

> ゲーム側がこの startCell を実際にどう開くか（ORACLE 開始モデル等）は **ゲーム挙動** であり、
> データ仕様の責務外（Architecture §11 Non Goals）。設計は `etc/V2_GENERATION_ENGINE.md` を参照。
