...
- [ ] **新規作成APIの実装**：`focusCell`（回転→ズーム→idle）、`isBusy`/`waitUntilIdle`、`getCellScreenPosition`、`loadScenarioBoard`、`onCellAction`（handleCellActionにフック）、`setInputMode`
- [ ] 固定盤面注入 `loadScenarioBoard` のための盤面データ形式の決定
- [ ] ハイライト追従：カメラ回転中に毎フレーム `getCellScreenPosition` を呼ぶ描画ループの実装方針
